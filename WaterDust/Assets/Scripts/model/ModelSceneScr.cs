...
public class ModelSceneScr
{
    ...
    public ModelSceneScr Init()
    {
        ...
        int xWidth = SizeWorld-1;
        int zWidth = SizeWorld-1;
    ...
    HyperCubeModel SetStoneRandom(HyperCubeModel hyperCubeItemCube)
    {
        ...
        {
            hyperCubeModel = GetNewCube(false, keySide.X, keySide.Y, keySide.Z);
        }
    ...
    */
    ...
    public HyperCubeModel GetNewCube(bool Water, int x, int y, int z)
    {
        indexCubeIncr++;
    ...
    bool MoveDownCube(HyperCubeModel hyperCubeItemCube, List<HyperCubeModel> addHyperCubeModelList, bool Stone)
    {
        ...
        if (downCubeId == 0)
        {
            ...
            {
                Debug.Log("================ 00   == =");
                HyperCubeModel hyperCubeModel = new StoneManager().SetStoneRandom(hyperCubeItemCube,this);

//  if (hyperCubeModel == null)
                if (hyperCubeItemCube.EqualKey(hyperCubeModel.GetPointCube()))
                {
            ...
            if (itemCubeValue.PathList.Count > 0)
            {
                ...
                if (_modelSceneScr.AllowMoveCube((int)itemCubeValue.PathList[itemCubeValue.PathList.Count - 1].X, (int)itemCubeValue.PathList[itemCubeValue.PathList.Count - 1].Y))
                {
                    ...
                    if (itemCubeValue.TakeStone)
                    {
                        ...
                        if (randomNum == 0)
                        {
                            HyperCubeModel hyperCubeModel = new StoneManager().SetStoneRandom(itemCubeValue,this);

                            if (itemCubeValue.EqualKey(hyperCubeModel.GetPointCube()))
                            {
